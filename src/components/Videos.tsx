...
const Videos: React.FC<VideosProps> = ({ isHome = false }) => {
  ...
  return (
    <section id="videos" className={`${isHome ? 'py-12 sm:py-16 md:py-20' : 'py-16 sm:py-24 md:py-32'} bg-white relative overflow-hidden`}>
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className={`flex flex-col ${isHome ? 'items-center text-center' : 'md:flex-row md:items-end justify-between'} ${isHome ? 'mb-8 md:mb-12' : 'mb-12 md:mb-20'} gap-8`}>
          <div className={`${isHome ? 'max-w-3xl' : 'max-w-2xl'}`}>
            <div className={`flex items-center ${isHome ? 'justify-center' : ''} gap-4 mb-4`}>
              ...
              <span className={`text-red-600 font-mono ${isHome ? 'text-xs' : 'text-sm'} tracking-widest uppercase`}>Video Archive</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 max-w-6xl mx-auto">
          {videoItems.map((video, index) => (
            <div
              key={index}
              className="relative bg-white border border-slate-100 p-4 transition-all duration-500 hover:border-red-600/50 shadow-sm hover:shadow-xl group"
            >
              {/* Decorative corner */}
              <div className="absolute -top-px -right-px w-6 h-6 bg-slate-50 border-b border-l border-slate-100"></div>
              
              <div className="relative aspect-video overflow-hidden bg-slate-50 mb-4">
                {video.id ? (
                  <iframe
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <p className="text-slate-300 font-mono text-[10px] uppercase tracking-widest text-center">
                      Video_ID_Required
                    </p>
                  </div>
                )}
              ...
              <div className="flex flex-col">
                <div className="text-[9px] font-mono text-slate-400 uppercase tracking-widest mb-1.5 flex justify-between">
                  ...
                  <span className="text-red-600 opacity-0 group-hover:opacity-100 transition-opacity font-bold text-[7px]">
                    PLAY_VIDEO
                  </span>
                </div>
                <h3 className="font-bold text-xl text-slate-900 leading-tight group-hover:text-red-600 transition-colors line-clamp-2 uppercase tracking-tighter">
